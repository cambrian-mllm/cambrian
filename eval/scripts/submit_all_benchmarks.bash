...
echo "> submit_all_benchmarks.bash $@"
...
for benchmark in "${benchmarks[@]}"; do
    ...
    if [[ -z "$dependency" ]]; then
        bash scripts/submit_eval.bash --ckpt $ckpt --benchmark $benchmark --conv_mode $conv_mode --gpus $gpus --constraint $constraint --cpus $cpus --mem $mem --time $time
    else
        bash scripts/submit_eval.bash --ckpt $ckpt --benchmark $benchmark --conv_mode $conv_mode --gpus $gpus --constraint $constraint --cpus $cpus --mem $mem --time $time --dependency $dependency
    fi
done
