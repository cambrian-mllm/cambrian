...
from cambrian.train.cambrian_trainer import LLaVATrainer
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'image' in sources[0]:
            image_files = dat['image']
            image_folder = self.data_args.image_folder
            processor = self.data_args.image_processor
   
            if not isinstance(image_files, list):
                image_files = [image_files]  # Ensure image_files is always a list
            ...
            for image_file in image_files:
                ...
                images_tensor_list.append(image)
        
            if images_tensor_list:
                # Concatenate all processed images along a new dimension
                image_tensor = torch.stack(images_tensor_list)
            else:
                # If there were no images, this part of the code may need adjustment depending on requirements
