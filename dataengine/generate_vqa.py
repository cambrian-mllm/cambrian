...
def main(topicname, path_to_images, qa_path, vqa_path):
    ...
    max_number = max((int(f.split('.')[0]) for f in existing_files if f.split('.')[0].isdigit() and f.endswith('.png')), default=0)
    ...
    img_files.sort(key=lambda x: (int(x.split('_')[1]), int(x.split('_')[2].split('.')[0])))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Process and rename images and update JSON data accordingly.')
    parser.add_argument('--topicname', type=str, default='Geology_and_Earth_Sciences', help='Name of the topic to process')
    parser.add_argument('--path_to_images', type=str, default='images', help='Path to the images directory')
    parser.add_argument('--qa_path', type=str, default='qadata', help='Path to the input qa JSON file')
    parser.add_argument('--vqa_path', type=str, default='vqadata', help='Path to the output vqa JSON file')
