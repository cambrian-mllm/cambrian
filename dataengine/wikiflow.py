...
def scrape_wikipedia(pagename, field, subfield, topic, url):    
    ...
    for i in range(len(headings)-1):
        ...
        section_title = heading.find(class_='mw-headline').text if heading.find(class_='mw-headline') else 'No title'        
        ...
        while element and element != next_heading:
            if element.name == 'p':
                ...
                if img and 'src' in img.attrs:
                    image_url = "https:" + img['src']  # Ensure the image URL is complete
                    ...
                    if caption_el is None:
                        ...
                        caption_el = element.find('div', class_=lambda x: x and 'thumbcaption' in x)
                    image_caption = caption_el.get_text(" ", strip=True) if caption_el else "No caption"
...
def read_links_from_json_file(file_path, topic):    
    with open(file_path, 'r') as file:
        data = json.load(file)        
    links = data.get(topic, [])  # Returns an empty list if the topic is not found    
    return links
...
def main(topics_directory, links_path, data_path): 
    ...
    json_files = [os.path.join(topics_directory, f) for f in os.listdir(topics_directory) if f.endswith('.json')]
    ...
    for json_file in json_files:
        ...
        file_base_name = os.path.splitext(os.path.basename(json_file))[0]  # Extract base name without extension    
        if not skip:
            for topic, subtopics in topics.items():
                for subtopic in subtopics:                                
                    ...
                    google_output = get_google_search_results(f"Wikipedia {subtopic}")                
                    if len(google_output)==0:
                        print(f'skiping {file_path} because of no results from google')
                        continue
                    preprocessed_google = preprocess_google(google_output)                
                    formatted_google_results = format_results_for_query(preprocessed_google)                
                    ...
                    for i, title in enumerate(titles):
                        dataset = scrape_wikipedia(title, file_base_name, topic, subtopic, links[i])
                        if dataset is not None:
                            totaldataset.extend(dataset)
                    write_datalinks_to_file(topic, subtopic, links, links_path)
                    append_data_to_file(topic, subtopic, totaldataset, file_path)
        else:
            for topic, subtopics in topics.items():
                for subtopic in subtopics:        
                    links = read_links_from_json_file(f'{links_path}{file_base_name}.json', subtopic) 
                    titles = [title for title in titles]
                    #print(links)
                    totaldataset = []
                    for title in titles:
                        dataset = scrape_wikipedia(title, file_base_name, topic, subtopic)
                        totaldataset.extend(dataset)
                    write_datalinks_to_file(topic, subtopic, links, links_path)
                    append_data_to_file(topic, subtopic, totaldataset, file_path)

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Process Wikipedia data for various topics.')
    parser.add_argument('--topics_directory', type=str, default='topics', help='Directory containing topics JSON files')
    parser.add_argument('--links_path', type=str, default='wikidata/wikilinks/', help='Directory to store links data')
    parser.add_argument('--data_path', type=str, default='wikidata/data/', help='Directory to store processed data')
